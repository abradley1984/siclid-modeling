...
def define_clfs_params():
    ...
    clfs = {'RF': RandomForestClassifier(n_estimators=50, n_jobs=-1),
        ...
        'SVM': svm.SVC(kernel='linear', probability=False, random_state=0),
        'GB': GradientBoostingClassifier(learning_rate=0.05, subsample=0.5, max_depth=6, n_estimators=10),
        'NB': GaussianNB(),
        'DT': DecisionTreeClassifier(),
        'SGD': SGDClassifier(loss="hinge", penalty="l2"),
        'KNN': KNeighborsClassifier(n_neighbors=3),
        'RFTEST': RandomForestClassifier(n_estimators=1, n_jobs=-1),
            }

    grid = { 
        'RFTEST':{'n_estimators': [1], 'max_depth': [1], 'max_features': ['sqrt'],'min_samples_split': [2]},
'RF':{'n_estimators': [1,10,100,1000], 'max_depth': [1,5,10,20,50], 'max_features': ['sqrt','log2'],'min_samples_split': [2,5,10]},
    'LR': { 'penalty': ['l1','l2'], 'C': [0.00001,0.0001,0.001,0.01,0.1,1,10]},
    'SGD': { 'loss': ['hinge','log','perceptron'], 'penalty': ['l2','l1','elasticnet']},
    'ET': { 'n_estimators': [1,10,100,1000], 'criterion' : ['gini', 'entropy'] ,'max_depth': [1,5,10,20,50], 'max_features': ['sqrt','log2'],'min_samples_split': [2,5,10]},
...
def clf_cv_loop(models_to_run, clfs, grid, X, y):
    ...
    for index,clf in enumerate([clfs[x] for x in models_to_run]):
        ...
        for p in ParameterGrid(parameter_values):
            try:
                ...
                #print (mean_score)
                #print (clf)
                tmp_series = pd.Series([str(clf),mean_score])
                #print ('Temp series:',tmp_series)
                results_df = results_df.append(tmp_series,ignore_index=True)
            except IndexError:
                print ('Error:')
                continue
